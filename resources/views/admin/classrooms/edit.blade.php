...
@section('title', 'Modifier une Salle - PFM')
@section('header_title', 'Modifier une Salle')
    ...
    <form action="{{ route('admin.classrooms.update', $classroom->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="code" id="code" required value="{{ old('code', $classroom->code) }}"
                    class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
            ...
            <div>
                ...
                <input type="text" name="floor" id="floor" value="{{ old('floor', $classroom->floor) }}"
                    class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>

            <div>
                <label for="capacity" class="block text-sm font-medium text-slate-700 mb-1.5">Capacité</label>
                <input type="number" name="capacity" id="capacity" min="1" value="{{ old('capacity', $classroom->capacity) }}"
                    class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    @foreach($statuses as $status)
                        <option value="{{ $status }}" {{ old('status', $classroom->status) === $status ? 'selected' : '' }}>
                            {{ $status }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <input type="text" name="equipment" id="equipment" value="{{ old('equipment', is_array($classroom->equipment) ? implode(', ', $classroom->equipment) : '') }}"
                class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
