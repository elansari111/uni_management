...
@section('title', 'Modifier un Niveau - PFM')
@section('header_title', 'Modifier un Niveau')

@section('content')
<div class="max-w-2xl bg-white border border-slate-100 rounded-3xl p-8 shadow-sm">
    <form action="{{ route('admin.levels.update', $level->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $level->name) }}"
            ...
            <div>
                ...
                <input type="text" name="code" id="code" required value="{{ old('code', $level->code) }}"
                    class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>

            <div>
                <label for="order" class="block text-sm font-medium text-slate-700 mb-1.5">Ordre</label>
                <input type="number" name="order" id="order" min="0" value="{{ old('order', $level->order) }}"
                    class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">{{ old('description', $level->description) }}</textarea>
        </div>
