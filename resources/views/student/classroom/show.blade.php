        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500">
                    {{ $module->teacher?->name ?? 'Professeur' }} • {{ $module->group?->name ?? 'Sans groupe' }}
                </p>
            </div>
            <a href="{{ route('student.modules.index') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-semibold transition-colors">
                Retour
            </a>
        </div>
    </div>
                ...
                @forelse($announcements as $ann)
                    <div class="p-6">
                        ...
                        <div class="mt-5">
                            ...
                            <form action="{{ route('student.announcements.comments.store', $ann->id) }}" method="POST" class="mt-4 flex gap-3">
                                @csrf
